//
//  PetProfileDetailHeaderView.swift
//  CollarandComb
//
...
@MainActor
struct PetProfileDetailHeaderView: View {
	enum Constants {
...
//				LazyImage(url: profile.header) { state in
//					if let image = state.image {
//						image
//							.resizable()
//							.aspectRatio(contentMode: .fill)
//							.overlay(profile.haveHeader ? .black.opacity(0.50) : .clear)
//							.frame(height: Constants.headerHeight)
//							.clipped()
...
//				.frame(height: Constants.headerHeight)
		...
		.frame(height: Constants.headerHeight)
		.onTapGesture {
//			guard profile.haveHeader else {
//				return
//			}
//			let attachement = MediaAttachment.imageWith(url: profile.header!)
...
//		.accessibilityHidden(profile.haveHeader == false)
	}

	private var accountAvatarView: some View {
		HStack {
			ZStack(alignment: .topTrailing) {
//				AvatarView(profile.avatar, config: .account)
//					.accessibilityLabel("accessibility.tabs.profile.user-avatar.label")
			...
			.onTapGesture {
//				guard profile.haveAvatar else {
//					return
//				}
//				let attachement = MediaAttachment.imageWith(url: profile.avatar!)
#if targetEnvironment(macCatalyst) || os(visionOS)
...
//			.accessibilityHidden(profile.haveAvatar == false)
...
struct AccountDetailHeaderView_Previews: PreviewProvider {
	static var previews: some View {
		AccountDetailHeaderView(viewModel: .init(user: .placeholder()),
								account: .placeholder(),
								scrollViewProxy: nil)
	}
}
