...
extension TimelineViewModel: ProductsFetcher {
...
//		return allProducts
	...
	func fetchProducts() {
		let url = URL(string: "\(Constants.nodeBaseURL)products")
		...
		let result = URLSession.shared.dataTask(with: urlReq) { (data, resp, err) in
			...
			do {
				...
				let response = try decoder.decode([Product].self, from: gotData)//
				DispatchQueue.main.async {
					self.productsState = .display(products: response, nextPageState: response.count < 20 ? .none : .hasNextPage)
					print("self.productsState ()")
				}
			} catch {
	...
	func fetchNextPage() async {
		guard let client else { return }
		do {
			print("fetchNextPage() let statuses")
...
//			ProductDataControllerProvider.shared.updateDataControllers(for: response.data, client: client)
			print("fetchNextPage() \(response.data.count)")

			productsState = await .display(products: response.data, nextPageState: response.data.count < 20 ? .none : .hasNextPage)
