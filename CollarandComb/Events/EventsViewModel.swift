...
@Observable class EventsViewModel {
	enum State {
		...
		case loading
		case display(events: [Event], nextPageState: State.PagingState)
...
//		}
	...
	func fetchEvents() async {
		...
		withAnimation {
			state = .display(events: Event.placeholders(), nextPageState: consolidatedNotifications.isEmpty ? .none : .hasNextPage)
		}
...
//		}
	...
	private func fetchNewPages(minId: String, maxPages: Int) async -> [Event] {
//		guard let client else { return [] }
//		var pagesLoaded = 0
