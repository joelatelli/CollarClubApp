...
//		.background(theme.secondaryBackgroundColor)
	...
	private func createStatusActionPicker(selection: Binding<ProductAction>, label: LocalizedStringKey) -> some View {
		Picker(selection: selection, label: Text(label)) {
			Section {
				Text(ProductAction.none.displayName()).tag(ProductAction.none)
			}
			Section {
				ForEach(ProductAction.allCases) { action in
					if action != .none {
