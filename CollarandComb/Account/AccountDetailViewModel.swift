...
@Observable class AccountDetailViewModel {
	...
	enum UserState {
		case loading, data(user: User), error(error: Error)
	}
	...
	var userState: UserState = .loading
	...
	private(set) var statuses: [Order] = []
...
//			let data = try await fetchAccountData(accountId: accountId, client: client)
			userState = .data(user: user ?? .placeholder())
