...
extension ProductEditor {
	...
	@Observable public class ViewModel: NSObject, Identifiable {
		...
		var mode: Mode
		var productDto: CreateProductData?
...
//			}
		...
		func postProduct() async -> Product? {
			...
			do {
				...
				let data = ProductData(title: statusText.string,
									   price: 5.50,
									   desc: statusText.string,
									   imageURL: "",
									   size: nil, 
									   quantity: 0,
									   isMenuItem: true,
				...
				switch mode {
				case .new:
					postProduct = try await client.post(endpoint: Products.postProduct(json: data))
					if let postProduct {
						StreamWatcher.shared.emmitPostEvent(for: postProduct)
					}
				case let .edit(product):
					postProduct = try await client.put(endpoint: Products.editProduct(id: product.id!.uuidString, json: data))
					if let postProduct {
						StreamWatcher.shared.emmitEditEvent(for: postProduct)
					}
				}
...
extension ProductEditor.ViewModel: DropDelegate {
	public func performDrop(info: DropInfo) -> Bool {
		let item = info.itemProviders(for: ProductEditor.UTTypeSupported.types())
...
extension ProductEditor.ViewModel: UITextPasteDelegate {
